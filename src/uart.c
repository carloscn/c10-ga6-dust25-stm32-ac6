...
UART* uart_new_dev(enum uart_id_t uart_num,uint32_t baud)
{
    ...
    self->msg.ring_io = ring_new_dev(UART_RECV_BUFFER_MAX, self->msg.buffer_rom);
    self->init          =   &uart_init;
...
void USART2_IRQHandler( void )
{
    ...
    {
    	recv[0] = USART_ReceiveData(USART2) & 0xFF;
    	dust_uart->msg.ring_io->master->push(dust_uart->msg.ring_io, (void*)recv, 1);
    }
